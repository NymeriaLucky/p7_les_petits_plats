...
//Init le tableau des appareils, ustensils, ingredients
const setArray = (recipes) => {
    recipes.forEach(r => {
        //Init la liste des appareils, n'envoie(push) uniquement s'il n'est pas présent dans (arrayOfAppliance)
        if(arrayOfAppliance.indexOf(r.appliance) === -1) arrayOfAppliance.push(r.appliance);
        //Init les listes d'ustanciles, n'envoie que s'il n'est pas inclus dans (arrayOfUstensils)
        r.ustensils.map((ustensil) => { 
            if(!arrayOfUstensils.includes(ustensil)) arrayOfUstensils.push(ustensil); 
        });    
        //Init les listes d' ingredients, n'envoie que s'il n'est pas inclus dans (arrayOfIngredients)
...
const setRecipesHTML = (recipes) => {
    ...
    recipes.forEach((r, index) => {
        ...
        `);
        //Initliste des ingrédients, n'appuyer que s'il n'est pas présent dans (arrayOfIngredients)
        //Ajouter un ingrédient HTML associé à l'ID de recette
        r.ingredients.forEach(i => $(`#id-card-ingredients-${r.id}`).append(`<li>${i.ingredient}: <span>${i.quantity ? i.quantity : ''} ${i.unit ? i.unit : ''}</span></li>`));
    });
};

//utiliser des recettes constantesJson, pour la prod
...
const filterByDropdownText = (inputElem, dropdownList) => {
    ...
    if(input.value.length >= 3){
        ...
        //Réinitialiser tout la liste déroulante
        ul = document.getElementById(dropdownList);
        li = ul.getElementsByTagName("li");
        for (i = 0; i < li.length; i++) {
...
$(".dropDownList").on("click", "li", function(event){
    ...
    if(!arrayOfTagId.includes(event.target.id)){
        arrayOfTagId.push(event.target.id); //Pour supprimer la balise
        arrayOfTagValue.push(event.target.text); //Pour filtrer la liste des recettes
...
//Afficher la liste par champ de texte
...
const filterAllByText = () => {
    ...
    if(searchValue.length >= 3) {
        ...
        //Rechercher une recette par appareil ou par nom
        ...
        //Rechercher une recette par ustensils
...
//Afficher et masquer les recettes plutôt que les balises
